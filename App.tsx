  ...
  const validateBarcodeUrl = useCallback(async (id: number, value: string) => {
    ...
    try {
      ...
      if (isFullUrl) {
        ...
        // It's a number; look it up via our Vercel Serverless Function.
        // The path `/api/get-barcode-url` is relative to our own domain.
        // Vercel automatically handles routing this to the function file.
      ...
      // Use HEAD request for a lightweight check without downloading the full image
      ...
      if (group.id === idToUpdate) {
        if (field === 'inputValue') {
          return {
              ...
              resolvedUrl: '', // Clear old URL
              validationStatus: 'idle', // Mark for re-validation
            };
        }
